...
def inference(args, stream):
    ...
    print("\ninference checkpoint KEYPOINT_PAINTER", keypoint_painter)
    print("\ninference checkpoint ANOTATION_PAINTER", annotation_painter)
    ...
    print("\ninference checkpoint animation: ", animation.video_output)
    ...
    if capture.isOpened():
        print("capture opened branch")
        online = True
        LOG.info('Loaded stream: ' + str(RTSPURL))
    else:
        print("NO CAPTURE branch")
        LOG.error('Cannot open stream: ' + str(RTSPURL))

    print("CHECKPOINT PASS IF ELSE")
    last_loop = time.time()
    output_fps = 0
    droppedFrames = 0
    old_fallcount = 0
